...
window.addEventListener('mousemove', (e) => {
    ...
    document.querySelectorAll('.tilt-element').forEach(el => {
        ...
        el.style.transform = `perspective(1000px) rotateX(${((e.clientY - r.top) / r.height - 0.5) * -10}deg) rotateY(${((e.clientX - r.left) / r.width - 0.5) * 10}deg) scale(1.01)`;
    });
});
...
onAuthStateChanged(auth, async (user) => {
    ...
    if (user) {
        ...
        let userData = { name: user.displayName, photo: user.photoURL || "https://img.icons8.com/color/96/user.png", phone: "", city: "", country: "", gender: "Male", customPhoto: "" };
        try { const docSnap = await getDoc(doc(db, "users", user.uid)); if(docSnap.exists()) userData = { ...userData, ...docSnap.data() }; } catch(e) { console.log(e); }
        ...
        document.getElementById('editName').value = userData.name; document.getElementById('editPhone').value = userData.phone;
        document.getElementById('editCity').value = userData.city; document.getElementById('editCountry').value = userData.country;
        document.getElementById('editGender').value = userData.gender; document.getElementById('customPhotoUrl').value = userData.customPhoto || "";
    } else {
...
window.saveUserProfile = async () => {
    ...
    const data = { name: document.getElementById('editName').value, phone: document.getElementById('editPhone').value, city: document.getElementById('editCity').value, country: document.getElementById('editCountry').value, gender: document.getElementById('editGender').value, customPhoto: document.getElementById('customPhotoUrl').value };
...
// --- NEWS (60 ITEMS, 6 CATEGORIES) ---
const newsGrid = document.getElementById('newsGrid');
const categories = ['AI', 'Tech', 'Gaming', 'Men', 'Women', 'Design'];
const newsData = [];
for(let i=1; i<=60; i++) {
    const cat = categories[i % 6];
    newsData.push({ id: i, cat: cat, img: `https://picsum.photos/400/600?random=${i}`, title: `${cat} Update #${i}`, desc: `Detailed details about ${cat} news item number ${i}. Stay updated with Kasun's Creator Hub.` });
}
function renderNews(filter) {
    ...
    newsData.forEach(item => {
        if(filter === 'all' || item.cat === filter) {
            ...
            el.innerHTML = `<img src="${item.img}"><div class="news-info-box"><span class="news-tag">${item.cat}</span><h3 style="font-size:0.9rem; margin-top:5px; color:white;">${item.title}</h3></div>`;
            el.onclick = () => openNewsPopup(item); newsGrid.appendChild(el);
        }
    });
}
renderNews('all');
window.filterNews = (f) => { document.querySelectorAll('.filter-btn').forEach(b => b.classList.remove('active')); event.target.classList.add('active'); renderNews(f); };
...
window.switchTab = (t) => { document.querySelectorAll('.tab-content').forEach(c => c.style.display = 'none'); document.getElementById(t+'Tab').style.display = 'block'; document.querySelectorAll('.tab-btn').forEach(b => b.classList.remove('active')); event.target.classList.add('active'); };
...
const words = ["Video Editor", "Photographer", "AI Artist", "Designer"]; let idx = 0;
